...
// version — reused here rather than EventFollowButton's rounded-full
// treatment because this sits in the same compact identity-row icon
// cluster the Business page uses). Guest path: email-capture modal ->
// POST /api/follow-location -> location_followers (via follow_location()
// RPC). Signed-in path: no email prompt, writes straight to
// account_followed_locations via POST /api/account/follow-location.
// Following a Location never touches account_followed_events/
// account_followed_businesses — entirely separate tables, no cross-follow.
export default function LocationFollowButton({
  locationId,
  locationSlug,
  locationName,
  size = "default",
  ...
  useEffect(() => {
    setFollowing(isLocationFollowed(locationId));

    let cancelled = false;
    getAccountSession().then((isAuthed) => {
      if (cancelled || !isAuthed) return;
      setAuthed(true);
      fetch(`/api/account/follow-location?slug=${encodeURIComponent(locationSlug)}`)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setStatus("loading");
    try {
      const res = await fetch("/api/follow-location", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ locationId, email: trimmed }),
      });
      if (res.ok) {
        markLocationFollowed(locationId);
        setFollowing(true);
        setOpen(false);
        setEmail("");
        setStatus("idle");
  ...
  async function handleAuthedFollow() {
    try {
      const res = await fetch("/api/account/follow-location", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ slug: locationSlug }),
      });
      if (res.ok) {
        ...
        createPortal(
          <div className="fixed inset-0 z-[70] flex items-end justify-center sm:items-center sm:p-4">
            ...
            <div
              role="dialog"
              aria-modal="true"
              aria-label={`Follow ${locationName}`}
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <h2 className="font-display text-lg font-bold tracking-tight text-ink">Follow {locationName}</h2>
                <button
                  type="button"
                  onClick={close}
                  aria-label="Close"
              ...
              <p className="mt-1.5 text-sm text-ink/60">
                Enter your email to follow this venue and get future updates.
              </p>
