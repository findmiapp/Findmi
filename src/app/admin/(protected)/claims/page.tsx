...
// Admin Claims + Marketplace Correctness pass — "Venue" is visible copy
// only; the underlying value stays "location" (ClaimEntityType, the
// location_claim_requests table, /admin/locations routes — all untouched).
...
const ENTITY_TYPE_LABEL: Record<ClaimEntityType, string> = {
  business: "Business",
  event: "Event",
  location: "Venue",
};
...
export default async function AdminClaimsPage({
  ...
  const { status, type, error, approved, rejected, member_updated } = await searchParams;
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-ink/50">
        Requests from Findmi accounts to manage an existing Business, Event, or Venue. Approving grants the
        appropriate ownership/management relationship, so review each claim carefully. Claiming a Business or Venue
        is free; Event management follows the existing Event Management entitlement — there is no separate Event
        claim fee.
      </p>
      ...
      <div className="mt-6 flex flex-col gap-3">
        {claims.length === 0 && (
          <p className="text-sm text-ink/50">
            No{activeType ? ` ${ENTITY_TYPE_LABEL[activeType]}` : ""}
            {activeStatus ? ` ${STATUS_VIEWS.find((v) => v.value === activeStatus)?.label.toLowerCase()}` : ""} claims
            {activeType || activeStatus ? " yet." : "."}
          </p>
        )}
        ...
        {claims.map((c) => {
          ...
          return (
            ...
            >
              ...
              <div className={`mt-2 grid gap-x-4 gap-y-1 sm:grid-cols-2 ${isPending ? "text-ink/70" : "text-ink/50"}`}>
                <p>Claimant: {c.claimantDisplayName || "—"}</p>
                <p>Email: {c.claimantEmail || "—"}</p>
                {c.claimantPhone && <p>Phone: {c.claimantPhone}</p>}
              </div>
              ...
              {c.entityAlreadyOwned && isPending && (
                <p className="mt-2 text-xs font-semibold text-amber-700">
                  This {ENTITY_TYPE_LABEL[c.entityType]} already has an owner — approving will be blocked until
              ...
              {isPending && (
                <div className="mt-3 flex gap-2">
                  <form action={approveClaim.bind(null, c.entityType, c.id)}>
                    <button
                      type="submit"
                      className="rounded-lg bg-findmi px-3.5 py-1.5 text-xs font-bold uppercase tracking-wide text-white transition hover:bg-findmi-600 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-findmi"
                    >
                      Approve
                    </button>
                  </form>
                  <form action={rejectClaim.bind(null, c.entityType, c.id)}>
                    <button
                      type="submit"
                      className="rounded-lg border border-black/10 px-3.5 py-1.5 text-xs font-bold uppercase tracking-wide text-ink/60 transition hover:bg-black/[0.03]"
                    >
                      Reject
                    </button>
                  </form>
