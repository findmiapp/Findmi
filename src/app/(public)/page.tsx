    ...
    <div>
      ...
      <section className="border-b border-black/5 bg-white">
        <div className="mx-auto flex max-w-6xl flex-col gap-3 px-4 py-4 sm:px-6 sm:py-5">
          ...
          {/* Subtle business doorway — secondary to the consumer headline
              above, never competing with it for attention. */}
          <Link
            href="/join"
      ...
      {/* Early business conversion module — right after the first discovery
          moment, not buried at the bottom. Reads as part of the product
          (a bordered card matching the rest of the page), not an inserted
          ad. This is the first of two $99 mentions on the page. */}
      <section className="mx-auto max-w-6xl px-4 py-6 sm:px-6">
        <div className="rounded-3xl border border-black/10 bg-white p-6 sm:p-8">
          <p className="text-xs font-bold uppercase tracking-wide text-findmi-700">Founding 500</p>
          <h2 className="mt-1 font-display text-xl font-semibold tracking-tight text-ink sm:text-2xl">
            Your business moves. FindMi moves with it.
          </h2>
          <p className="mt-2 max-w-md text-sm text-ink/65">
            One profile for what you sell, where you&rsquo;ll be next, and how customers can reach you.
          </p>
          <p className="mt-4 flex items-baseline gap-1.5">
            <span className="font-display text-2xl font-bold tracking-tight text-ink">$99</span>
            <span className="text-sm text-ink/50">/year</span>
          </p>
      ...
      {/* What a FindMi profile actually does — the "See what you get"
          destination from the early module above. Three pillars, not a
          features table. */}
      <section id="for-business" className="mx-auto max-w-6xl scroll-mt-16 px-4 py-8 sm:px-6">
        ...
        <p className="mt-2 max-w-lg text-sm text-ink/65">
          Brands, food trucks, makers, market vendors, and mobile businesses use FindMi to stay
          findable wherever they show up next.
        </p>
        <div className="mt-6 grid gap-5 sm:grid-cols-3">
          <BusinessPillar
            title="Show what you sell"
            items={["Products & services", "Photos", "Customer inquiries"]}
          />
          <BusinessPillar
            title="Show where you'll be"
            items={["Markets & pop-ups", "Events", "Retail & appearances"]}
          />
          <BusinessPillar
            title="Get discovered"
            items={["FindMi Here", "Events & categories", "Search & followers"]}
          />
        </div>
      </section>
      ...
      {/* Closing business CTA — the second and last $99 mention. The early
          module introduced the opportunity; this closes it. */}
...
function BusinessPillar({ title, items }: { title: string; items: string[] }) {
  return (
